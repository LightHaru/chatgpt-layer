...
function resolvedAccountsRoot(userRoot2) {
  return (0, import_node_path12.resolve)(accountsRoot(userRoot2));
}
function assertStrictlyInsideAccountsRoot(userRoot2, target) {
  const root = resolvedAccountsRoot(userRoot2);
  const dir = (0, import_node_path12.resolve)(target);
...
function sessionDir(userRoot2, id) {
  assertSessionId(id);
  return assertStrictlyInsideAccountsRoot(userRoot2, (0, import_node_path12.join)(resolvedAccountsRoot(userRoot2), id));
}
...
function collectForbiddenDeleteTargets(userRoot2) {
  const out = [
    ...
    resolvedAccountsRoot(userRoot2)
  ];
  if (process.env.HOME) {
...
function isForbiddenSessionDeleteTarget(userRoot2, target) {
  ...
  return collectForbiddenDeleteTargets(userRoot2).some((path) => path === resolved);
}
function rmSessionDir(userRoot2, id) {
  const accounts = resolvedAccountsRoot(userRoot2);
  const dir = sessionDir(userRoot2, id);
  let target = dir;
  if ((0, import_node_fs15.existsSync)(dir)) {
    const stat4 = (0, import_node_fs15.lstatSync)(dir);
    target = stat4.isSymbolicLink() ? (0, import_node_fs15.realpathSync)(dir) : (0, import_node_fs15.realpathSync)(dir);
  }
  const resolvedAccounts = (0, import_node_fs15.existsSync)(accounts) ? (0, import_node_fs15.realpathSync)(accounts) : accounts;
  ...
  if (isForbiddenSessionDeleteTarget(userRoot2, target)) {
...
var CodexSessionManager = class {
  ...
  createSession(input = {}) {
    ...
    (0, import_node_fs17.mkdirSync)(accountsRoot(this.userRoot), { recursive: true });
    try {
  ...
  allocateId() {
    for (let i = 0; i < 8; i++) {
      ...
      if (!this.records.has(id) && !(0, import_node_fs17.existsSync)((0, import_node_path14.join)(accountsRoot(this.userRoot), id))) return id;
    }
